        ...
        {data.items && data.items.map((item, id) => (
          <div className='p-4 text-center bg-white shadow shadow-slate-200 rounded'>
            <div className='flex justify-center'>
              <img className='h-48' src={testimage} alt=""/>
            </div>
            <p className='text-3xl text-primary font-medium line-clamp-1'>{item.name}</p>
            ...
            {cart.includes(item) ? <HandleCount item={item}/> : <AddToCart item={item} />}
          </div>
        ))}
      </div>
    </div>
