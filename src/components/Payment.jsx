  ...
  return (
    <div className='bg-white w-3/4 rounded-2xl p-6 '>
      ...
      <div className='flex flex-col space-y-2'>
        {stores && stores.map((store, index) => (
          <div id={index} className='flex space-x-2'>
            <input onClick={() => handleClick(store)} className={isClicked[store] ? activeRadio : inActiveRadio} name='store' type="radio"/>
            <label className={isClicked[store] ? activeRadioLabel : inActiveRadioLabel} htmlFor='store'>{store[0].toUpperCase() + store.slice(1, store.length)}: {storeTotal[store]}₾</label>
          </div>
        ))}
      </div>
      <GrayLine attribute='my-6'/>
      ...
      <div className='relative'>
        <input className='bg-gray-100 w-full p-4 rounded-3xl font-bold' type="text" placeholder='Add promo'/>
        <button className='absolute bg-primary text-handleCountBackground font-medium py-4 px-9 rounded-3xl right-0 shadow shadow-lg'>Apply</button>
      </div>
      <GrayLine attribute='my-6'/>
      {/* all costs */}
      <div className='text-lg space-y-3'>
        <div className='flex justify-between'>
          ...
          <p className='text-primary font-bold'>{storeTotal[clickedStore] ? '₾ ' + storeTotal[clickedStore] : 'Choose your store'}</p>
        </div>
        <div className='flex justify-between'>
